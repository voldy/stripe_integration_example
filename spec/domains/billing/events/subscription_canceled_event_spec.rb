...
RSpec.describe Billing::Events::SubscriptionCanceledEvent, type: :model do
  ...
  describe '#initialize' do
    ...
    it 'assigns the payload correctly' do
      expect(event.payload).to eq({ subscription_id: 'sub_123', customer_id: 'cus_123', status: 'canceled' })
    end
  ...
  describe '#to_s' do
    it 'returns a string representation of the event' do
      expect(event.to_s).to eq("SubscriptionCanceledEvent: #{event.payload}, version: 1.0, timestamp: #{timestamp}, id: #{uuid}")
    end
  end
end
